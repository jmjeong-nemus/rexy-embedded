...
static UNITCL *frame_buffer = NULL;
...
* @if HISTORY
*			25-Apr-2006	Kwanho Shin modified
*/
bool RalTimeGetCurrent(struct tm* outCurrentTime)
{
    ...
    pTime.tm_year = time.wYear - 1900;
	pTime.tm_mon = time.wMonth - 1;
	pTime.tm_mday = time.wDay;
	pTime.tm_hour = time.wHour;
	pTime.tm_min = time.wMinute;
	pTime.tm_sec = time.wSecond;
    pTime.tm_wday = time.wDayOfWeek;
...
* 
* @if HISTORY
*			25-Apr-2006	Kwanho Shin modified - WM_PAINT 관련 부분 추가
*/
    ...
    while(1)
    {
        ...
        else
        {
            ...
            else if ( tmsg.message == WM_TIMER )
			{
				 ...
				 outMsg->wparam = tmsg.wParam;
				 outMsg->lparam = 0;
 				 ret = KillTimer( g_hMainWnd, tmsg.wParam );
				 return TRUE;
			}
        }
    }
...
// Created by Joshua to get faster speed on windows based simulator
// 12-Apr-2007
void CreateDIB(void)
{
    ...
    //binfo_header.
    g_bInfo = (BITMAPINFO *)calloc(1, sizeof(BITMAPINFOHEADER) + sizeof(DWORD) * 3);
    g_bInfo->bmiHeader = binfo_header;

    // for BPP_16
    ((DWORD *)g_bInfo->bmiColors)[0] = 0xF800;
    ((DWORD *)g_bInfo->bmiColors)[1] = 0x07E0;
    ((DWORD *)g_bInfo->bmiColors)[2] = 0x001F;

    hdc = GetDC(g_hMainWnd);

 	frame_bitmap = CreateDIBSection(hdc, g_bInfo, DIB_RGB_COLORS, &p, NULL, 0);
 	frame_buffer = p;	

	ReleaseDC(g_hMainWnd,hdc);
}

void DestroyDIB(void)
{
    if (frame_bitmap)
    {
...
*			25-Apr-2006	Kwanho Shin modified
*/
static void PrvDumpScreen(rect* inBoundary)
{
    InvalidateRect(g_hMainWnd, inBoundary, FALSE);
...
bool RalScreenFlush(void)
{
	...
	Boundary.left = 0;
	Boundary.top = 0;
	Boundary.right = LCD_RIGHT;
	Boundary.bottom = LCD_BOTTOM;

    memcpy( frame_buffer, lcd_bmp_buffer, sizeof(lcd_bmp_buffer) );
    PrvDumpScreen(&Boundary);
    return TRUE;
}
...
bool RalScreenFlushPart(rect* inBoundary)
{
    ...
    for( y = sy ; y <= ey ; y++ )
    {
		memcpy((frame_buffer + y * DEVICE_WIDTH + sx ), &lcd_bmp_buffer[y][sx], sizeof( UNITCL ) * ( ex - sx + 1 ));
    }
    PrvDumpScreen(inBoundary);
    return TRUE;
}
/** Repaint screen.
*
