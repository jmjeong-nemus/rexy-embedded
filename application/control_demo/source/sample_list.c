...
static void PrvListCallback(WND wnd, int nMsg, int wparam, int lparam, int bIsPre, int *pErr);
static void PrvDrawListItemFocus(WND wnd, int line, void *pData);
static void PrvDrawListItem(WND wnd, int line, void *pData);
...
static void PrvDrawImage(WND wnd, int line, int col, int start_x, int* pErr);
...
//	CtlSetiPtr(w, CTL_LIST_CMD_SET_COLUMN_MARGIN, 3, (void*)&margin_text,&err);
	...
	CtlSetiPos(w, CTL_LIST_CMD_SET_TEXT_ALIGN, ALIGN_LEFT, ALIGN_LEFT, &err);
...
static void PrvListCallback(WND wnd, int nMsg, int wparam, int lparam, int bIsPre, int *pErr)
{
	...
	{
	case AWM_CREATE:
		// Handle the task associated with creation
		break;
	case AWM_PAINT:
		if(bIsPre==TRUE)
		{
			// Do paint the something that will be drawn BEFORE control drawing
		}
		else	// bIsPre==FALSE
		{
			// Do paint the something that will be drawn AFTER control drawing
		}
		break;
	}
}
...
static void PrvDrawImage(WND wnd, int line, int col, int start_x, int* pErr)
{
	int img_x, img_y;
	COLORVAL old_bcolor;
	int nTopIndex = CtlGeti(wnd, CTL_LIST_CMD_GET_TOPINDEX, pErr);
	RxBITMAP* bitmapS = &g_bmpListNumA[line + nTopIndex];
	RxBITMAP* bitmapN = &g_bmpListNumN[line + nTopIndex];

	REXY_CTL_LIST* pCtlList = (REXY_CTL_LIST*)CtlGetStruct(gpListWnd);

	img_x = start_x + pCtlList->column_margin[col].left;
	img_y = line * pCtlList->nLineHeight + (line * pCtlList->nLineInterval) + pCtlList->column_margin[0].top;
...
//	if(line + nTopIndex==1 || line + nTopIndex==2)
//	{
//		switch(column) 
//		{
//		case 0:
//			PrvDrawImage(wnd, line, column, 0, &err);
//			break;
//		case 1:
//			PrvDrawImage(wnd, line, column, 30, &err);
//			break;
//		case 2:
//			PrvDrawImage(wnd, line, column, 60, &err);
//			break;
//		}
//	}
//	else
